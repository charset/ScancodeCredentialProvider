...
CSampleCredential::CSampleCredential(): _cRef(1), _pCredProvCredentialEvents(NULL) {
    DllAddRef();
...
HRESULT CSampleCredential::Initialize(__in CREDENTIAL_PROVIDER_USAGE_SCENARIO cpus,
    __in const CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR* rgcpfd, __in const FIELD_STATE_PAIR* rgfsp) {
    HRESULT hr = S_OK;
    ...
    if (SUCCEEDED(hr)) {
        hr = SHStrDupW(L"Scan QrCode", &_rgFieldStrings[SFI_LARGE_TEXT]);
    }
    if (SUCCEEDED(hr)) {
        hr = SHStrDupW(L"Choose to Scan", &_rgFieldStrings[SFI_SMALL_TEXT]);
    }
    if (SUCCEEDED(hr)) {
        hr = SHStrDupW(L"CHARSET", &_rgFieldStrings[SFI_EDIT_TEXT]);
    }
    if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
        hr = SHStrDupW(L"How To Use?", &_rgFieldStrings[SFI_COMMAND_LINK]);
    }
...
HRESULT CSampleCredential::SetSelected(__out BOOL* pbAutoLogon) {
	if(wcslen(_rgFieldStrings[SFI_EDIT_TEXT]) > 0 && wcslen(_rgFieldStrings[SFI_PASSWORD]) > 0) 
		*pbAutoLogon = TRUE;
	else 
		*pbAutoLogon = FALSE;  
    return S_OK;
...
// to display the tile.
HRESULT CSampleCredential::GetFieldState(__in DWORD dwFieldID, __out CREDENTIAL_PROVIDER_FIELD_STATE* pcpfs,
...
HRESULT CSampleCredential::GetCheckboxValue(__in DWORD dwFieldID, 
    __in BOOL* pbChecked, __deref_out PWSTR* ppwszLabel) {
...
HRESULT CSampleCredential::GetComboBoxValueCount(__in DWORD dwFieldID, 
...
HRESULT CSampleCredential::GetComboBoxValueAt(__in DWORD dwFieldID, 
...
HRESULT CSampleCredential::CommandLinkClicked(__in DWORD dwFieldID) {
    ...
    if (dwFieldID < ARRAYSIZE(_rgCredProvFieldDescriptors) && 
        ...
        ::MessageBox(hwndOwner, L"1. Open your WECHAT;\n\n2. Scan the QrCode above;\n\n3. Contact administrator if failure;", L"Into!", 0);
        hr = S_OK;
    } else {
        hr = E_INVALIDARG;
    }
